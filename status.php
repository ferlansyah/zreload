    ...
    else
    {
        ...
        if ($trx->op_produk == 'token_pln')
            echo '<tr><td>ID PLN</td><td>' . substr($trx->tr_id_pln, 0, -4) .
                'XXXX</td></tr>';
        if ($trx->op_produk == 'voucher_game')
            echo '<tr><td>Email</td><td>' . substr($trx->tr_email, -10, -5) .
                '----</td></tr>';
        ...
        {
            echo '<div class="alert alert-danger"><h3>Pembayaran Kadarluarsa</h3>Kami belum menerima pembayaran & transaksi sudah tidak berlaku<br>Silahkan ulangi pesanan anda.</div>';
        }
        ...
        echo '<div class="hidden-print"><a class="btn btn-default" href="#"' .
            ' onclick="window.print();return false;"><i class="glyphicon glyphicon-print"></i> Cetak Transaksi</a> &nbsp; ' .
            '<a class="btn btn-warning" href="' . SITE_URL . 'hubungi_kami.php?inv_id=' . $trx->
            tr_id_pembayaran . '" target="_blank"><i class="glyphicon glyphicon-exclamation-sign"></i> Komplain Transaksi</a></div>';
...
else
{
    ...
    $history = $q->fetchAll();
    echo '<h3>Daftar Status Transaksi</h3>';
...
<div>
  ...
  <ul>
    <li>
    Jika transaksi Gagal atau status RF (Refund) / <i>dikembalikan</i> kami akan menghubungi anda via SMS untuk metode pengembalian dana anda.
    </li>
  </ul>
</div>
<?php
